import type { AuthContext } from '../index';
import { Hono } from 'hono';
import { getUser } from '../utils/roles';
import { db, schema } from '../db';
import { eq, and, or, isNull, desc } from 'drizzle-orm';

const { personalKpi, personalKpiHistory, users } = schema;

export const personalKpiRoutes = new Hono<AuthContext>();

// Get personal KPI for current user
personalKpiRoutes.get('/', async (c) => {
  try {
    const { userId } = getUser(c);
    const { year, month } = c.req.query();

    let conditions = [eq(personalKpi.userId, userId)];

    if (year) {
      conditions.push(eq(personalKpi.year, parseInt(year)));
    }
    if (month) {
      conditions.push(eq(personalKpi.month, parseInt(month)));
    }

    const kpis = await db
      .select({
        id: personalKpi.id,
        userId: personalKpi.userId,
        year: personalKpi.year,
        month: personalKpi.month,
        incomeTarget: personalKpi.incomeTarget,
        commissionRate: personalKpi.commissionRate,
        conversionRate: personalKpi.conversionRate,
        avgOrderValue: personalKpi.avgOrderValue,
        workingDaysPerMonth: personalKpi.workingDaysPerMonth,
        createdAt: personalKpi.createdAt,
        updatedAt: personalKpi.updatedAt,
        userName: users.name,
      })
      .from(personalKpi)
      .leftJoin(users, eq(personalKpi.userId, users.id))
      .where(and(...conditions))
      .orderBy(desc(personalKpi.year), desc(personalKpi.month));

    return c.json({ success: true, data: kpis });
  } catch (err) {
    console.error('Get personal KPI error:', err);
    return c.json({ success: false, error: 'Lỗi server' }, 500);
  }
});

// Create or update personal KPI
personalKpiRoutes.put('/', async (c) => {
  try {
    const { userId } = getUser(c);
    const data = await c.req.json();

    const currentYear = data.year || new Date().getFullYear();
    const currentMonth = data.month || new Date().getMonth() + 1;

    // Validate working_days_per_month [C5]
    if (data.working_days_per_month !== undefined) {
      if (data.working_days_per_month < 20 || data.working_days_per_month > 31) {
        return c.json({ success: false, error: 'Số ngày làm việc phải từ 20 đến 31' }, 400);
      }
    }

    // Check if KPI exists for this year/month
    const existing = await db
      .select()
      .from(personalKpi)
      .where(
        and(
          eq(personalKpi.userId, userId),
          eq(personalKpi.year, currentYear),
          currentMonth != null
            ? eq(personalKpi.month, currentMonth)
            : isNull(personalKpi.month)
        )
      )
      .limit(1);

    const now = new Date();

    if (existing[0]) {
      // Update existing
      await db
        .update(personalKpi)
        .set({
          incomeTarget: data.income_target ?? existing[0].incomeTarget,
          commissionRate: data.commission_rate ?? existing[0].commissionRate,
          conversionRate: data.conversion_rate ?? existing[0].conversionRate,
          avgOrderValue: data.avg_order_value ?? existing[0].avgOrderValue,
          workingDaysPerMonth: data.working_days_per_month ?? existing[0].workingDaysPerMonth,
          updatedAt: now,
        })
        .where(eq(personalKpi.id, existing[0].id));

      // Save history [C10 extension]
      await db.insert(personalKpiHistory).values({
        id: crypto.randomUUID(),
        personalKpiId: existing[0].id,
        userId: userId,
        year: currentYear,
        month: currentMonth,
        incomeTarget: data.income_target ?? existing[0].incomeTarget,
        commissionRate: data.commission_rate ?? existing[0].commissionRate,
        conversionRate: data.conversion_rate ?? existing[0].conversionRate,
        avgOrderValue: data.avg_order_value ?? existing[0].avgOrderValue,
        workingDaysPerMonth: data.working_days_per_month ?? existing[0].workingDaysPerMonth,
        changedBy: userId,
        changedAt: now,
      });

      const updated = await db
        .select()
        .from(personalKpi)
        .where(eq(personalKpi.id, existing[0].id))
        .limit(1);

      return c.json({ success: true, data: updated[0] });
    } else {
      // Create new
      const kpiId = crypto.randomUUID();

      await db.insert(personalKpi).values({
        id: kpiId,
        userId: userId,
        year: currentYear,
        month: currentMonth || null,
        incomeTarget: data.income_target || null,
        commissionRate: data.commission_rate || null,
        conversionRate: data.conversion_rate || null,
        avgOrderValue: data.avg_order_value || null,
        workingDaysPerMonth: data.working_days_per_month || 26,
        createdAt: now,
        updatedAt: now,
      });

      const created = await db
        .select()
        .from(personalKpi)
        .where(eq(personalKpi.id, kpiId))
        .limit(1);

      return c.json({ success: true, data: created[0] }, 201);
    }
  } catch (err) {
    console.error('Update personal KPI error:', err);
    return c.json({ success: false, error: 'Lỗi server' }, 500);
  }
});

// Calculate KPI - returns computed values
personalKpiRoutes.get('/calculate', async (c) => {
  try {
    const { income_target, commission_rate, conversion_rate, avg_order_value, working_days_per_month } = c.req.query();

    if (!income_target) {
      return c.json({ success: false, error: 'Thiếu mục tiêu thu nhập' }, 400);
    }

    const income = parseFloat(income_target);
    const commission = (parseFloat(commission_rate) || 0) / 100;
    const conversion = (parseFloat(conversion_rate) || 0) / 100;
    const aov = parseFloat(avg_order_value) || 0;
    const workingDays = parseInt(working_days_per_month) || 26;

    if (workingDays < 20 || workingDays > 31) {
      return c.json({ success: false, error: 'Số ngày làm việc phải từ 20 đến 31' }, 400);
    }

    // Calculations [C5][M5.2]
    // Sales target = Income target / Commission rate
    const salesTarget = commission > 0 ? income / commission : 0;

    // Number of orders = Sales target / Avg order value
    const numOrders = aov > 0 ? salesTarget / aov : 0;

    // Potential customers needed = Number of orders / Conversion rate
    const potentialCustomers = conversion > 0 ? numOrders / conversion : 0;

    // Daily prospects = Ceiling(potential customers / working days) [C5]
    const dailyProspects = Math.ceil(potentialCustomers / workingDays);

    return c.json({
      success: true,
      data: {
        income_target: income,
        commission_rate: commission * 100,
        conversion_rate: conversion * 100,
        avg_order_value: aov,
        working_days_per_month: workingDays,
        sales_target: Math.round(salesTarget),
        num_orders: Math.round(numOrders),
        potential_customers_per_month: Math.round(potentialCustomers),
        potential_customers_per_day: dailyProspects,
      },
    });
  } catch (err) {
    console.error('Calculate KPI error:', err);
    return c.json({ success: false, error: 'Lỗi server' }, 500);
  }
});
